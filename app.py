...
def admin_dashboard():
    ...
    # Prepare verified submissions
    verified_submissions = {
        username: {title: data for title, data in user_submissions.items() if data['verified']}
        for username, user_submissions in submissions.items()
    }

    # Calculate completion rates
    completion_rates = calculate_completion_rates(students, submissions, problems)

    # Get unique problems
    unique_problems = get_unique_problems(problems)

    # Prepare unique problems as a list of HTML strings for rendering
    unique_problems_html = ''.join(
        f'<li><a href="{link}" target="_blank" class="text-blue-500 hover:text-blue-700">{title}</a></li>'
        for title, link in unique_problems
    )
    ...
    return render_template(
        ...
        unique_problems=unique_problems_html,  # Pass the prepared HTML string
        **metrics
    )
...
'''
def verify_solution_with_ocr(image_path, expected_link):
    """Verify solution using OCR.space API with enhanced error handling and logging"""
    try:
        logger.info(f"Starting verification for image: {image_path}")
        ...
        logger.debug(f"First 200 chars of extracted text: {extracted_text[:200]}")

        # Extract and verify problem link
        extracted_link = extract_problem_link(extracted_text)
        normalized_expected = normalize_url(expected_link)
        normalized_extracted = normalize_url(extracted_link) if extracted_link else ''

        logger.info(f"Normalized expected link: {normalized_expected}")
        logger.info(f"Normalized extracted link: {normalized_extracted}")

        # Check URL match and problem status
        url_matches = normalized_extracted and (
            normalized_extracted in normalized_expected or
            normalized_expected in normalized_extracted
        )

        is_solved = check_problem_solved(extracted_text)

        logger.info(f"URL matches: {url_matches}")
        logger.info(f"Is solved: {is_solved}")

        verification_details = {
            'link_matched': url_matches,
            'extracted_link': extracted_link,
            'expected_link': expected_link,
            'problem_status': 'SOLVED' if is_solved else 'NOT SOLVED',
            'verification_method': 'OCR.space API',
            'extracted_text': extracted_text[:500]  # First 500 chars for debugging
        }

        is_verified = url_matches and is_solved
        logger.info(f"Final verification result: {is_verified}")
        ...
        return False, {
            'error': str(e),
            'verification_method': 'OCR.space API',
            'verification_stage': 'verification_process'
        }
'''
'''
...
def submit_solution():
    try:
        logger.debug("Starting submission process")
        ...
        # Get problem title from form
        problem_title = request.form.get('problem_title')
        if not problem_title:
            logger.error("No problem title provided")
            flash('No problem specified', 'error')
            return redirect(url_for('student_dashboard'))
        ...
        filename = secure_filename(f"{username}_{problem_title}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}")
        ...
        if not problem:
            logger.error(f"Problem not found: {problem_title}")
            flash('Problem not found', 'error')
            return redirect(url_for('student_dashboard'))

        # Verify the submission
        logger.debug(f"Verifying solution for problem: {problem_title}")
        is_verified, verification_details = verify_solution_with_ocr(image_path, problem['link'])
...
def extract_problem_link(text):
    ...
    patterns = [
        ...
        r'problems\/[a-zA-Z0-9-]+\/\d+',
        r'geeksforgeeks\.org\/[a-zA-Z0-9-]+\/\d+',
        # Add more flexible pattern for problem URLs
        r'practice\.geeksforgeeks\.org\/problems?\/[a-zA-Z0-9-]+(?:\/\d+)?',
        r'leetcode\.com/problems/[a-zA-Z0-9-]+/submissions/?',
        r'hackerrank\.com/challenges/[a-zA-Z0-9-]+/problem/?',
    ...
    logger.debug(f"Searching for URL patterns in text: {text[:500]}...")  # Log first 100 chars
